...
export const ProductForm = ({ productId, onClose, onSuccess }: ProductFormProps) => {
    ...
    const fallbackRandomId = useRef(Math.random().toString(36).substring(2, 8));
    ...
    useEffect(() => {
        ...
        const uniqueSlug = formData.sku ? `${baseSlug}-${formData.sku.toLowerCase()}` : `${baseSlug}-${fallbackRandomId.current}`;
        ...
        const autoSeoTitle = formData.name ? `${formData.name} - Buy Online | Balaki Organic` : '';
        const autoAltText = formData.name ? `Product image of ${formData.name}` : '';
        const autoMetaDesc = `Buy ${formData.name || 'this product'} online at the best price from Balaki Organic. 100% authentic ${formData.category || 'items'} with fast delivery across Pakistan.`;

        setFormData(prev => {
            const updates: any = {};
            
            // Auto-sync SEO fields if empty or matching auto templates
            if (!prev.seo_title || prev.seo_title === lastAutoGeneratedRef.current.seo_title || prev.seo_title.includes('Buy Online')) {
                updates.seo_title = autoSeoTitle;
            }
            if (!prev.meta_description || prev.meta_description === lastAutoGeneratedRef.current.meta_description || prev.meta_description.includes('online at the best price')) {
                updates.meta_description = autoMetaDesc;
            }
            if (!prev.alt_text || prev.alt_text === lastAutoGeneratedRef.current.alt_text || prev.alt_text.includes('Product image of')) {
                updates.alt_text = autoAltText;
            }
            if (!prev.tags || prev.tags === lastAutoGeneratedRef.current.tags) {
                updates.tags = autoTags;
            }
            if (!prev.slug || prev.slug === lastAutoGeneratedRef.current.slug) {
                updates.slug = uniqueSlug;
            }

            // Update ref for the next change
            lastAutoGeneratedRef.current = {
                seo_title: updates.seo_title || prev.seo_title,
                meta_description: updates.meta_description || prev.meta_description,
                alt_text: updates.alt_text || prev.alt_text,
    ...
    useEffect(() => {
        ...
        const loadProduct = async () => {
            ...
            try {
                ...
                // Initialize ref with loaded values to allow sync on name change
                lastAutoGeneratedRef.current = {
                    seo_title: data.seo_title || '',
                    meta_description: data.meta_description || '',
                    alt_text: data.alt_text || '',
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const updatedData = {
                ...
                condition_note: formData.is_used ? formData.condition_note : null
            };
            ...
            while (attempts < 10) {
                ...
                // Check if error is missing column error: "Could not find the 'xyz' column of 'products'"
                const match = error.message?.match(/Could not find the '([^']+)' column/i);
                if (match && match[1]) {
                    const missingCol = match[1];
                    console.warn(`⚠️ Column '${missingCol}' not found in products table schema. Pruning '${missingCol}' and retrying...`);
            ...
            >
                <div className="p-8 border-b border-white/5 flex items-center justify-between bg-white/[0.02] backdrop-blur-xl">
                    <div>
                        ...
                        <p className="text-xs font-bold opacity-40 uppercase tracking-widest mt-1 text-primary">Balaki Organic Admin Portal</p>
                    </div>
                ...
                <form onSubmit={handleSubmit} className="p-8 overflow-y-auto custom-scrollbar space-y-8 flex-grow">
                    <div className="space-y-6">
                        ...
                        <div className="space-y-4">
                            <div>
                                ...
                                {formData.slug && (
                                    <p className="mt-2 text-[10px] font-black opacity-30 uppercase tracking-widest px-2">
                                        URL Slug: <span className={formData.slug === (formData.sku ? `${slugify(formData.name)}-${formData.sku.toLowerCase()}` : slugify(formData.name)) ? "text-primary/60" : "text-amber-500/80"}>
                                            {formData.slug}
                                        </span>
                                        {formData.slug !== (formData.sku ? `${slugify(formData.name)}-${formData.sku.toLowerCase()}` : slugify(formData.name)) && (
                                            <span className="ml-2 text-amber-500 opacity-100 font-black cursor-help" title="Manually edited or diverged from Name"> (DIVERGED)</span>
                                        )}
                                    </p>
                                )}
                            </div>
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div className="flex items-center gap-4 bg-foreground/[0.03] border border-foreground/10 p-4 rounded-2xl">
                                    <div className={`w-12 h-6 rounded-full relative cursor-pointer transition-colors border ${formData.is_returnable ? 'bg-primary border-primary' : 'bg-foreground/10 border-foreground/10'}`}
                                ...
                                <div className="flex items-center gap-4 bg-foreground/[0.03] border border-foreground/10 p-4 rounded-2xl">
                                    <div className={`w-12 h-6 rounded-full relative cursor-pointer transition-colors border ${formData.is_free_delivery ? 'bg-green-500 border-green-500' : 'bg-foreground/10 border-foreground/10'}`}
                                    ...
                                    <div className="flex-grow">
                                        <p className="text-xs font-black uppercase tracking-tight">Free Delivery</p>
                                        <p className="text-[10px] opacity-40 font-bold uppercase tracking-wider">Zero Shipping Fee</p>
                                    </div>
                                </div>

                                <div className="flex items-center gap-4 bg-foreground/[0.03] border border-foreground/10 p-4 rounded-2xl">
                                    <div className={`w-12 h-6 rounded-full relative cursor-pointer transition-colors border ${formData.is_used ? 'bg-accent border-accent' : 'bg-foreground/10 border-foreground/10'}`}
                                        onClick={() => setFormData(prev => ({ ...prev, is_used: !prev.is_used }))}>
                                        <motion.div
                                            animate={{ x: formData.is_used ? 26 : 4 }}
                                            className="absolute top-1 w-4 h-4 bg-white rounded-full shadow-md"
                                        />
                                    </div>
                                    <div className="flex-grow">
                                        <p className="text-xs font-black uppercase tracking-tight">Second Hand</p>
                                        <p className="text-[10px] opacity-40 font-bold uppercase tracking-wider">Used Product</p>
                                    </div>
                                </div>
                            </div>
                            ...
                            {formData.is_used && (
                                <motion.div
                                    ...
                                    className="space-y-2"
                                >
                                    <label className="text-[10px] font-black uppercase tracking-[0.2em] opacity-30 px-1">Condition Note</label>
                                    <textarea
                                        value={formData.condition_note}
                                        onChange={e => setFormData({ ...formData, condition_note: e.target.value })}
                                        className="w-full bg-white/[0.03] border border-white/5 rounded-2xl p-4 outline-none focus:border-accent/50 transition-colors font-medium resize-none h-20"
                                        placeholder="e.g. Minor scratches on back, 10/10 condition, with box..."
                                    />
                                </motion.div>
                            )}
                        </div>
                                                            ...
                                                            <td className="p-2">
                                                                {row.image_url ? (
                                                                    ...
                                                                    <button
                                                                        ...
                                                                        className="flex items-center gap-1.5 text-[10px] font-black text-primary hover:scale-105 transition-all bg-primary/10 px-3 py-1.5 rounded-lg border border-primary/20 shadow-sm"
                                                                    >
